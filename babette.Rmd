...
beast_result_gtr <- babette::bbt_run_from_model(fasta_filename = "dna.fas",
                                            inference_model = inf_model,
                                            beast2_options = beastier::create_beast2_options(rng_seed = 4321))
```
...
par(mfrow = c(1, 2))
plot(cons_tree, main = "JC69")
plot(cons_tree_gtr, main = "GTR")

ape::ltt.plot(cons_tree)
ape::ltt.lines(cons_tree_gtr, col = "blue")
```

It seems the data is so strongly divergent, that it overwhelms 
choices about the substitution model.
...
We again remove the faulty trees as well, and plot the distribution of trees:
```{r burnin_mrca_trees}
all_trees <- beast_result$dna_trees
